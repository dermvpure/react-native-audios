...
@implementation AudioRecorderManager {
  ...
  NSURL *_amrAudioFileURL;
  BOOL _needConvertToAmr;
}
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder successfully:(BOOL)flag {
  ...
  NSString *fileUrl = [_audioFileURL absoluteString];
  if (_needConvertToAmr) {
    NSString *amrFile = _amrAudioFileURL.path;
    NSString *wavFile = _audioFileURL.path;
    wave_file_to_amr_file([wavFile cStringUsingEncoding:NSASCIIStringEncoding],
                          [amrFile cStringUsingEncoding:NSASCIIStringEncoding],
                          1,
                          16);
    fileUrl = amrFile;
  }
...
//      _audioEncoding =[NSNumber numberWithInt:kAudioFormatAMR];
      //generate wav, will convert to amr
      _audioEncoding = [NSNumber numberWithInt:kAudioFormatLinearPCM];
      _needConvertToAmr = YES;
      _amrAudioFileURL = _audioFileURL;
      _audioFileURL = [NSURL fileURLWithPath:[[_amrAudioFileURL.path stringByDeletingPathExtension] stringByAppendingPathExtension:@"wav"]];
      _audioSampleRate = [NSNumber numberWithFloat:8000];
    }
  }
